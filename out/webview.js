...
var hepmcViewer = (() => {
  ...
  // src/common/hepmc.ts
  function formatParticleTooltip(particle) {
  ...
  var state = window.__HEPMC_INITIAL_STATE__;
  var currentIndex = 0;
  render();
  window.addEventListener("message", (event) => {
    ...
    if (message.type === "update") {
      state = message.state;
      currentIndex = Math.min(currentIndex, Math.max(0, state.events.length - 1));
      render();
    }
  });
  function render() {
    ...
    if (!viewerState || viewerState.events.length === 0) {
      root2.appendChild(createEmptyState());
      return;
    }
  ...
  function createSvg(canvas, event, tooltip) {
    ...
    const zoomBehavior = zoom_default2().scaleExtent([0.2, 5]).on("zoom", (event2) => {
      select_default2(content).attr("transform", event2.transform.toString());
    });
    select_default2(svg).call(zoomBehavior);
  ...
  function renderNode(node, width, height, tooltip) {
  ...
  function toAbsPoint(node, width, height) {
    return {
      x: width / 2 + node.x,
      y: height / 2 + node.y
    };
